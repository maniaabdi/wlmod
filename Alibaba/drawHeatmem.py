...
memUsage = np.zeros([4035, 2305], dtype=float)
...
with open(path_to_csv_machine) as machineFile, open(path_to_csv_time) as timeFile, open(path_to_csv_1) as csv1File:
    for lineMachine, lineTime, lineMem in zip(machineFile, timeFile, csv1File):
        ...
        memUsage[countM, countT] = float(lineMem)
...
axcpu = sns.heatmap(memUsage,cmap='jet')
